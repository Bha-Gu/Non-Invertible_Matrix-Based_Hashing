...
def interlace(input):
    ...
    for i in range(8):
        tmp[2 * i], tmp[2 * i + 1] = (
            input[2 * i] + input[2 * i + 1] % 256,
            input[2 * i] * input[2 * i + 1] % 256,
        )

    for i in range(4):
        tmp[4 * i], tmp[4 * i + 2] = (
            tmp[4 * i] + tmp[4 * i + 2] % 256,
            tmp[4 * i] * tmp[4 * i + 2] % 256,
        )
        tmp[4 * i + 1], tmp[4 * i + 3] = (
            tmp[4 * i + 1] + tmp[4 * i + 3] % 256,
            tmp[4 * i + 1] * tmp[4 * i + 3] % 256,
        )

    for i in range(2):
        for j in range(4):
            tmp[8 * i + j], tmp[8 * i + j + 4] = (
                tmp[8 * i + j] + tmp[8 * i + j + 4] % 256,
                tmp[8 * i + j] * tmp[8 * i + j + 4] % 256,
            )

    for i in range(8):
        tmp[i], tmp[i + 8] = (
            input[i] + input[i + 8] % 256,
            input[i] * input[i + 8] % 256,
        )
    ...
    for i in range(8):
        out[i % 4][i // 4], out[i % 4, (i // 4) + 2] = interlace8(
            tmp[i % 4, i // 4], out[i % 4, i // 4 + 2]
        )
...
def interlace_from_string(input_str):
